...
const NewCourseForm = () => {
	...
	const [editData, _setEditData] = useState<CourseType | undefined>(
		location?.state
	);
	...
	const [formData, setFormData] = useState<DefaultValues>({
		...
		courseDescription: editData ? editData?.courseDescription : "",
		courseDuration: editData ? Number(editData?.courseDuration) : 0,
		courseInstructor: editData ? editData?.courseInstructor : "",
		topics: editData ? editData?.topics : [""],
		courseLevel: editData ? editData?.courseLevel : "beginner",
		skillsGain: editData ? editData?.skillsGain : [""],
		coursePrice: editData ? editData?.coursePrice : "",
		thumbnail: null,
	...
	const updateCourse = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		const {data} = await axios.patch(
			...
			{formData, courseId: editData?._id},
	...
	return (
		<div className="w-full h-full bg-white rounded-lg shadow-md p-5 overflow-y-auto dark:bg-slate-900 dark:border dark:border-slate-200 dark:border-opacity-20 hide-scrollbar">
			...
			<form
				onSubmit={editData ? updateCourse : handleSubmit}
				...
				<div>
					<Select
						value={editData ? editData.courseInstructor : ""}
						onValueChange={instructorChange}>
				...
				<div>
					<Select
						value={editData ? editData.courseLevel : ""}
						onValueChange={levelChange}>
						<SelectTrigger className="dark:bg-slate-900">
				...
				<div className="space-y-2">
					...
					<Input
						...
						required
						className="w-fit dark:bg-slate-900 dark:text-slate-200"
						onChange={(e) => handleChange(e)}
					/>
				</div>
					...
					<div key={sessionIndex} className="w-full h-auto space-y-5">
						...
						<div className="w-auto h-auto relative space-y-0">
							<Input
								...
								value={Number(session?.sessionDuration)}
								onChange={(e) => handleChange(e, sessionIndex)}
								required
								className="input-area bg-transparent"
							/>
